...
string SomeipSDControllerApp::dumpFinalConfig()
{
    stringstream dump;
    dump << "\"finalConfig\": { ";
    dump << dumpAllServices();
    dump << ", " << dumpAllIdleSlopes();
    dump << " }";
    return dump.str();
}

string SomeipSDControllerApp::dumpAllServices()
{
    stringstream dump;
    dump << "\"services\": [ ";
    bool first = true;
    for (auto& service: serviceTable->getServiceTable())
    {
        for (auto& instance: service.second)
        {
            ...
            // find all subscriptions for this service instance
            serviceTable->getSubscriptions(service.first, instance.first, false);
            bool firstSubscription = true;
            for (auto& sub: serviceTable->getSubscriptions(service.first, instance.first, false))
            {
...
string SomeipSDControllerApp::dumpAllIdleSlopes()
{
    stringstream dump;
    dump << "\"switches\": [ ";
    bool first = true;
    auto switches = controller->getSwitchesList();
    for (auto sw: *switches)
    {
        string swMac = sw.getMacAddress();
        if(!first)
        {
            dump << ",";
        }
        ...
        { 
            ...
            {
                SwitchPort swPort (swMac, port);
                auto slope = getPortIdleSlope(swPort, pcp);
                if (slope > 0)
                {
                    if(!firstPcp)
                    {
