...
#include <sdn4core/switch/avb/engine_oldVersion/Legacy_AVB_OF_RelayUnit.h>
...
Define_Module(LegacyAVB_OF_RelayUnit);

LegacyAVB_OF_RelayUnit::LegacyAVB_OF_RelayUnit() {
}

LegacyAVB_OF_RelayUnit::~LegacyAVB_OF_RelayUnit() {
}

void LegacyAVB_OF_RelayUnit::forwardSRPtoController(cPacket* msg) {
...
bool LegacyAVB_OF_RelayUnit::isSRPMessage(cMessage* msg) {
    return msg->arrivedOn("srpIn") || dynamic_cast<CoRE4INET::SRPFrame *>(msg);
}

void LegacyAVB_OF_RelayUnit::initialize(int stage){
    ...
    switch(stage){
        case INITSTAGE_LOCAL: {
            ...
            if(!_srpTable){
                throw cRuntimeError(("LegacyAVB_OF_RelayUnit: Could not init as no SRP Table could be found at " + par("srpTable").str()).c_str());
            }
    ...
    //    case INITSTAGE_ROUTING_PROTOCOLS:
        ...
        case INITSTAGE_LAST: {
            ...
            if(!srpProtocol){
                throw cRuntimeError(("LegacyAVB_OF_RelayUnit: Could not init as no SRP Protocol Module could be found at " + par("srpProtocolModule").str()).c_str());
            }
...
void LegacyAVB_OF_RelayUnit::handleMessage(cMessage *msg)
{
...
void LegacyAVB_OF_RelayUnit::handleSRPFromController(cMessage* msg) {
...
void LegacyAVB_OF_RelayUnit::handleSRPFromProtocol(cMessage* msg) {
...
void LegacyAVB_OF_RelayUnit::processDataPlanePacket(cMessage *msg){
    if (isSRPMessage(msg)) {
        dataPlanePacket++;
...
void LegacyAVB_OF_RelayUnit::processControlPlanePacket(cMessage *msg){
...
openflow::oxm_basic_match LegacyAVB_OF_RelayUnit::extractMatch(
...
void LegacyAVB_OF_RelayUnit::finish() {
    // record statistics
