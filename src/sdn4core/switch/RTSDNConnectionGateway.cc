...
void RTSDNConnectionGateway::initialize()
{
...
bool RTSDNConnectionGateway::isSRPMessage(cMessage* msg) {
    return msg->arrivedOn("encapSrpIn") || dynamic_cast<CoRE4INET::SRPFrame *>(msg);
}

void RTSDNConnectionGateway::handleMessage(cMessage *msg)
{
...
void RTSDNConnectionGateway::handleSRPMessage(cMessage* msg) {
    // srp message --> forward to correct modules
...
void RTSDNConnectionGateway::handleSwitchMessage(cMessage* msg) {
    //this message should be forwarded to the upper layer!
    EV_INFO << "Forwarding to upperLayer.\n";
    send(msg->dup(), "upperLayerOut");
}

void RTSDNConnectionGateway::handleOutMessage(cMessage* msg) {
    //from switch --> forward to correct buffer
