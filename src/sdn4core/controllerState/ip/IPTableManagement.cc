...
Define_Module(IPTableManagement);

bool IPTableManagement::update(openflow::Switch_Info* sw_info,
...
int IPTableManagement::lookup(openflow::Switch_Info* sw_info,
...
bool IPTableManagement::loadConfig(cXMLElement* configuration) {
    Enter_Method
    ("loadConfig");
    bool changed = false;
    if (configuration) {
...
bool IPTableManagement::loadConfigForSwitch(const string& swMacAddr,
...
void IPTableManagement::dumpConfigToStream(std::ostream& stream,
        int indentTabs) {
    Enter_Method
    ("dumpConfigToStream");
    string indent = string(indentTabs, '\t');
    stream << indent << "<macManager>" << endl;
    const map<string, FriendIPv4RoutingTable*>& managedStates =
            getAllManagedStates();
...
void IPTableManagement::onCreateManagedState(
        FriendMACAddressTable* managedState, string& swMacAddr) {
    ControllerStateManagementBase<FriendIPv4RoutingTable>::onCreateManagedState(
